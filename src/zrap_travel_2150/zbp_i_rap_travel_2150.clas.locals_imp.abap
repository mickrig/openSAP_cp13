...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zi_rap_travel_2150 IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF zi_rap_travel_2150 IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
...
*      " Clear state messages that might exist
    ...
    DATA business_data TYPE TABLE OF zrap_2150z_travel_agency_es5.
    ...
    IF agencies IS NOT INITIAL.
      ...
      TRY.
          ...
          NEW zcl_ce_rap_agency_2150( )->get_agencies(
            EXPORTING
...
**      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
      IF travel-AgencyID IS INITIAL OR NOT line_exists( business_data[ agencyid = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        %state_area = 'VALIDATE_AGENCY'
                        %msg        = NEW zcm_rap_2150(
                                          severity = if_abap_behv_message=>severity-error
                                          textid   = zcm_rap_2150=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_2150(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_2150=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_2150(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_2150=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_2150(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_2150=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
  ...
  METHOD get_instance_authorizations.
    ...
    is_update_requested = COND #(
                            WHEN requested_authorizations-%update              = if_abap_behv=>mk-on
                              ...
                              OR requested_authorizations-%action-Prepare      = if_abap_behv=>mk-on
                              OR requested_authorizations-%action-Edit         = if_abap_behv=>mk-on
                              OR requested_authorizations-%assoc-_Booking      = if_abap_behv=>mk-on
                              THEN abap_true
                            ELSE abap_false ).
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF is_update_requested EQ abap_true.
        ...
        IF has_before_image EQ abap_true.
          ...
          IF update_granted EQ abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_2150( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_2150=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
      ...
      APPEND VALUE #( %tky = travel-%tky
                      %update              = COND #(
                                               WHEN update_granted EQ abap_true
                      ...
                      %delete              = COND #(
                                               WHEN delete_granted EQ abap_true
                                                 THEN if_abap_behv=>auth-allowed
                                               ELSE if_abap_behv=>auth-unauthorized )
                    )
        TO result.
  ...
  METHOD is_create_granted.
    AUTHORITY-CHECK OBJECT 'ZOSTAT2150'
      ID 'ZOSTAT2150' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    IF has_before_image EQ abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTAT2150'
        ID 'ZOSTAT2150' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTAT2150'
        ID 'ZOSTAT2150' DUMMY
