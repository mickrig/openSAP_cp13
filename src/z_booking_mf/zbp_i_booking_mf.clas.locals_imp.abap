CLASS lhc_buffer DEFINITION.
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_buffer.
             INCLUDE TYPE ztbooking_mf AS data.
...
CLASS lhc_ZI_BOOKING_MF DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_ZI_BOOKING_MF IMPLEMENTATION.
  ...
  METHOD modify.
    ...
    IF roots_to_create IS NOT INITIAL.
      ...
      SELECT SINGLE MAX( booking ) FROM ztbooking_mf INTO @DATA(lv_max_booking).
    ENDIF.
    ...
    IF roots_to_update IS NOT INITIAL.
      LOOP AT roots_to_update INTO DATA(ls_update).
        ...
        IF sy-subrc <> 0.
          ...
          SELECT SINGLE * FROM ztbooking_mf WHERE booking = @ls_update-booking INTO @DATA(ls_db).
  ...
  METHOD read.
    LOOP AT it_booking_key INTO DATA(ls_booking_key).
      ...
      ELSE.
        SELECT SINGLE * FROM ztbooking_mf WHERE booking = @ls_booking_key-booking INTO @DATA(ls_db).
        IF sy-subrc = 0.
...
CLASS lsc_ZI_BOOKING_MF DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
